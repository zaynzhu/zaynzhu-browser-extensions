...
### 云盘助手：连接、分享转存与磁力下载（0.10.0）

需要 Chrome 116 及以上版本。

1. 加载 `extensions/cloud-drive-helper/`，点击扩展图标打开独立配置页。
2. 115 选择客户端后手机扫码；光鸭普通账号使用手机号验证码登录，官网连接作为备用；123 使用手机号或邮箱与密码登录。
3. 选择并保存各云盘的目标目录。115 与 123 选择根目录第一层文件夹，光鸭支持进入子目录；开发者凭证仅用于光鸭目录浏览。需要选择更深的目录时，开启默认关闭的「扫描全部文件夹目录」，扫描完成后按完整路径选择。
4. 右键分享链接或包含链接与提取码的选中文字，选择转存；右键单条磁力时明确选择目标云盘。
5. 在原生任务侧栏查看状态；切换标签页后仍保留，也可从配置页「全部任务」打开。服务端完成且目标目录回读核验通过才显示成功。
