...
### 云盘助手：统一配置与目录选择与分享转存（0.9.0）

1. 加载或重新加载 `extensions/cloud-drive-helper/`，点击扩展图标直接打开独立配置页；「全部任务」按钮打开浏览器原生任务侧栏。页面提供 115、光鸭、123 三个入口，光鸭默认使用网页登录，支持 115 手机扫码、光鸭双方式连接和 123 账号密码登录。
...
开发者凭证与目标选择保存在 `chrome.storage.local`，不随账号同步、不是加密保险库，且限制为扩展可信页面访问。官网备用连接只在主动点击连接时，通过 `scripting` 读取本扩展打开的光鸭官网标签页中指定的登录项；不读取其他标签页，不保存密码、验证码或刷新令牌，无日志或遥测。光鸭连接使用 `storage`、`scripting` 及光鸭的 `www`、`api`、`dapi` 三个 HTTPS 主机。API 请求至少间隔 2 秒，失败不自动重试。分享转存已完成真实验收；磁力为新增功能，真实验收另行记录。
...
短信登录实现遵循官网的验证初始化、发送验证码、校验验证码和登录顺序。发送间隔至少 60 秒；出现人机验证就停止，不绕过、不自动重复发短信。续期只轮换同账号的令牌，保留目标目录和连接身份；换账号失败保留旧连接。**短信登录与续期目前通过合成测试，尚未实际发送短信或完成真实账号登录验收。**
...
2026-09-10 第二条用户磁力在 123、光鸭完成并回读目标目录。115 在 0.7.2 曾因插件自行添加的历史查重上限而未提交；0.7.3 已按用户纠正移除此错误前置条件：只核对账号与已选目录，然后直接提交一次，由 115 创建接口判断已存在或失败，不重试、不移动旧任务。提交后仅从最近任务页定位本次哈希，不遍历历史；暂未找到只表示尚未确认完成。115 新建实际验收仍待更新记录。
